...
class _ShiftFrameEdit extends State<ShiftFrameEdit> {
  ...
  Future<List> loadShift() async {
    List<ShiftFrameModel> shiftFrames = await ClShiftFrame()
        .loadActiveShiftFrames(context, organId,
    ...
    if (shiftFrameId != null) {
      activeGroups =
          await ClShiftFrame().loadShiftFrameGroups(context, shiftFrameId!);

      tickets = await ClShiftFrameTickets().loadShiftFrameTickets(context, {'shift_frame_id' : shiftFrameId});
    }

    masterTickets = await ClTicket().loadMasterTicket(context, globals.companyId);
    for (TicketMasterModel me in masterTickets) {
      ShiftFrameTicketModel? st = tickets.firstWhereOrNull((element) => element.ticketId == me.id);
      
      frameTickets.add(ShiftFrameTicketModel.fromJson({
        'id' : '',
        'shift_frame_id' : '',
        'ticket_id' : me.id,
...
print(param);
  ...
  Future<void> deleteShift() async {
    ...
    bool isDelete =
        await ClShiftFrame().deleteShiftFrame(context, shiftFrameId!);
    if (isDelete) {
      Navigator.of(context).pop();
    } else {
    ...
    return Column(
      children: [
        ...
        RowLabelInput(
            ...
            label: '日付',
            renderWidget:
                Text(DateFormat('yyyy-MM-dd').format(widget.selection))),
        _getTimeRow(),
        RowLabelInput(
            hMargin: 20,
            label: 'シフト枠',
        ...
        if (isFrameMode)
          RowLabelInput(
              ...
              label: 'メモ',
              renderWidget:
                  TextInputNormal(controller: txtComment, multiLine: 3)),
            ...
            child: Column(children: [
              ...
              RowLabelInput(
                  ...
                  renderWidget: DropDownModelSelect(
                    value: staffId,
                    items: [
                      DropdownMenuItem(child: Text('選択なし'), value: null),
                      ...staffs.map((e) => 
                          DropdownMenuItem(child: Text(e.staffNick), value: e.staffId))
                  ], tapFunc: (v) => { staffId = v })),
              SizedBox(height: 12),
              RowLabelInput(
                  hMargin: 20,
