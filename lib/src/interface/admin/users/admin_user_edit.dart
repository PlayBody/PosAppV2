...
class _AdminUserEdit extends State<AdminUserEdit> {
  ...
  Future<List> loadUserInfo() async {
    ...
    if (results['isLoad']) {
      ...
      goldLevel = user['user_is_gold'] == null ? 0 : int.parse(user['user_is_gold']);
      ...
      if (ranks.isNotEmpty){
        ...
        maxStamp = int.parse(ranks.firstWhere((element) => element.level==userRank).maxStamp);
      }
      // ignore: use_build_context_synchronously
      stampCnt = await ClCommon().loadUserStampCount(context, widget.userId);
  ...
  Future<void> saveUserInfo() async {
    ...
    if (userNickController.text == '') {
      Fluttertoast.showToast(
          msg: 'ニックネームを入力してください。', toastLength: Toast.LENGTH_SHORT);
      return;
    } else {
      errUserNick = null;
    }
                          ...
                          children: [
                            ...
                            _getRowContent(
                                ...
                                DropDownModelSelect( 
                                  ...
                                  maxStamp = int.parse(ranks.firstWhere((element) => element.level==v).maxStamp);
                                  if (stampCnt > maxStamp) stampCnt = maxStamp;
                                  userRank = v;
                                  setState(() {});
                                })),
                            ...
                            _getRowContent(
                                'スタンプ',
                                DropDownNumberSelect(value: stampCnt.toString(), min:0, max: maxStamp, tapFunc: (v){ stampCnt = int.parse(v); setState(() {
