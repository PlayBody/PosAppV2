...
class AdminUserEdit extends StatefulWidget {
  final String userId;
  const AdminUserEdit({required this.userId, super.key});

  @override
  State<AdminUserEdit> createState() => _AdminUserEdit();
}
...
class _AdminUserEdit extends State<AdminUserEdit> {
  ...
  var userNickController = TextEditingController();
  ...
  Future<List> loadUserInfo() async {
    ...
    await Webservice()
        .loadHttp(context, apiLoadUserInfoUrl, {'user_id': widget.userId})
        .then((value) => results = value);

    if (results['isLoad']) {
      var user = results['user'];
      firstNameController.text = user['user_first_name'] ?? '';
      lastNameController.text = user['user_last_name'] ?? '';
      userNickController.text = user['user_nick'] ?? '';
      userSex = user['user_sex'] ?? '1';
      companyId = user['company_id'] ?? '';

      phoneController.text = user['user_tel'] ?? '';
      mailController.text = user['user_email'] ?? '';
      commentController.text = user['user_comment'] ?? '';

      if (user['user_birthday'] != null && user['user_birthday'] != '') {
        var birthdayDateTime = DateTime.parse(
          user['user_birthday'] + ' 00:00:00',
        );
      ...
      goldLevel =
          user['user_is_gold'] == null ? 0 : int.parse(user['user_is_gold']);
      ...
      if (mounted) {
        userTickets = await ClTicket().loadUserTickets(
          context,
          widget.userId,
          companyId,
        );
      ...
      if (mounted) {
        ranks = await ClCommon().loadRanks(context, companyId);
        if (ranks.isNotEmpty) {
          userRank = user['user_grade'] ?? '1';
          maxStamp = int.parse(
            ranks.firstWhere((element) => element.level == userRank).maxStamp,
          );
        }
        if (mounted) {
          stampCnt = await ClCommon().loadUserStampCount(
            context,
            widget.userId,
          );
          if (stampCnt > maxStamp) stampCnt = maxStamp;
  ...
  Future<void> saveUserInfo() async {
    ...
    Map<dynamic, dynamic> results = {};
    await Webservice()
        .loadHttp(context, apiSaveUserInfoUrl, {
          'user_id': widget.userId,
          ...
          'user_nick': userNickController.text,
          'user_sex': userSex,
          ...
          'user_grade': userRank,
          'stamp_count': stampCnt.toString(),
          'user_is_gold': goldLevel.toString(),
                      ...
                      child: SingleChildScrollView(
                        child: Column(
                          children: [
                            ...
                            _getRowContent(
                              'メールアドレス',
                              AdminTextInputNormal(controller: mailController),
                            ),
                            ...
                            _getRowContent(
                              'グループ分け',
                              AdminCommentText(label: groupName),
                            ),
                            AdminLineH1(),
                            _getRowContent(
                              '満タン回数',
                              DropDownNumberSelect(
                                value: goldLevel.toString(),
                                min: 0,
                                max: 99,
                                tapFunc: (v) {
                                  goldLevel = int.parse(v.toString());
                                },
                              ),
                            ),
                            AdminLineH1(),
                            ...
                            _getRowContent(
                              'スタンプ',
                              DropDownNumberSelect(
                                value: stampCnt.toString(),
                                min: 0,
                                max: maxStamp,
                                tapFunc: (v) {
                                  stampCnt = int.parse(v);
                                  setState(() {});
                                },
                              ),
                            ),
                            ...
                            Container(
                              ...
                              child: SingleChildScrollView(
                                child: Column(
                                  children: [
                                    ...orders.map(
                                      (e) => AdminUserEditHistoryItem(
                                        order: e,
                                        delFunc: () => deleteReserve(e.orderId),
                                      ),
                                    ),
                                  ],
                                ),
              ...
              children: [
                Flexible(
                  child: DropDownNumberSelect(
                    ...
                    max: 99,
                    min: 0,
                    value: int.parse(item.maxCount) >= 0 ? item.maxCount : '0',
                    tapFunc:
                        item.isInfinityCount
                            ? null
                            : (v) {
                              item.maxCount = v;
                              setState(() {});
            ...
            renderWidget:
                item.isInfinityCount
                    ...
                    : DropDownNumberSelect(
                      ...
                      min: 0,
                      value:
                          int.parse(item.count) > int.parse(item.maxCount)
                              ? item.maxCount
                              : item.count,
            ...
            renderWidget:
                item.isInfinityCount
                    ...
                    : DropDownNumberSelect(
                      ...
                      value:
                          int.parse(item.resetCount) >= int.parse(item.maxCount)
                              ? item.maxCount
                              : item.resetCount,
                      plusnum: 10,
                      max: int.parse(item.maxCount),
...
class AdminUserEditHistoryItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color:
            order.status == constOrderStatusReserveApply
                ? Color(0xffCECECE)
                : Colors.white,
            ...
            children: [
              Expanded(
                child: Container(
                  ...
                  child: Text(order.organName, style: styleItemGroupTitle),
                ),
              ),
              Container(
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          Text(
                            DateFormat(
                              'yyyy年MM月dd日(${weekAry[DateTime.parse(order.fromTime).weekday - 1]})',
