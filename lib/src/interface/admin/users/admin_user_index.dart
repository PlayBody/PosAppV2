...
class AdminUserIndex extends StatefulWidget {
  ...
  const AdminUserIndex({super.key, required this.userIds});

  @override
  State<AdminUserIndex> createState() => _AdminUserIndex();
}

class _AdminUserIndex extends State<AdminUserIndex> {
  final Logger _logger = Logger('AdminUserIndex');
  ...
  Future<List> loadinitData() async {
    ...
    if (searchFromBirthMonth != null) {
      strFromBirthday =
          '${int.parse(searchFromBirthMonth!) < 10 ? '0' : ''}${searchFromBirthMonth!}-';

      if (searchFromBirthDay == null) {
        strFromBirthday += '01';
      } else {
        strFromBirthday +=
            (int.parse(searchFromBirthDay!) < 10 ? '0' : '') +
            searchFromBirthDay!;
      }
    }
    ...
    if (searchToBirthMonth != null) {
      strToBirthday =
          '${int.parse(searchToBirthMonth!) < 10 ? '0' : ''}${searchToBirthMonth!}-';
    ...
    if (searchEnteringYear != null) {
      ...
      if (searchEnteringMonth == null) {
        ...
        strVisitday +=
            '${int.parse(searchEnteringMonth!) < 10 ? '0' : ''}${searchEnteringMonth!}-';
        if (searchEnteringDay == null) {
          strVisitday += '01';
        } else {
          strVisitday +=
              (int.parse(searchEnteringDay!) < 10 ? '0' : '') +
              searchEnteringDay!;
        }
      }
    }
    _logger.info('Visit day: $strVisitday');
    ...
    if (mounted) {
      ...
      for (var el in allUserList) {
        if (widget.userIds.isNotEmpty) {
          int searchIndex = widget.userIds.indexWhere(
            (element) => element == el.userId,
          );
          if (searchIndex > -1) {
            userList.add(el);
          }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          ...groupList.map(
            (e) => _getGroupListItem(e.groupId, e.userCnt!, e.groupName),
          ),
        ],
      ),
    );
      ...
      title: Container(
        ...
        child: Column(
          children: [
            Row(
              children: [
                Expanded(child: Text(groupName, style: styleItemGroupTitle)),
                Text('(${cnt ?? 0})', style: styleItemGroupTitle),
              ],
            ),
            Container(
  ...
  Widget _getUserListItem(UserModel e) {
    var userNick = e.userNick;
    var userName =
        e.userFirstName == null
            ? ' '
      ...
      child: ElevatedButton(
        ...
        child: Container(
          ...
          child: Row(
            children: [
              Expanded(
                child: Text('${userNick ?? userName}様', style: styleUserName1),
              ),
              Text(
                '${e.reserveCount!}/${e.visitCount!}',
