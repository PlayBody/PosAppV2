...
class _AdminMesseageMake extends State<AdminMesseageMake> {
  ...
  Future<void> selectPhoto() async {
    var select = await SelectAttachments().selectImageWithFile();
    if (select['file_path'] == null) return;
    attachType = '1';
    filePath = select['file_path'];
    fileName = select['file_name'];
    setState(() {});
  }

  Future<void> selectVideo() async {
    var select = await SelectAttachments().selectFileMovie();
    if (select['file_path'] == null) return;
    attachType = '2';
    filePath = select['file_path'];
    fileName = select['file_name'];
    videoPath = select['video_file'];
    setState(() {});
  }
  ...
  Widget _getChatAttachContent(attach) {
    TaskInfo? task = findAttachTask(attach);
    DownloadTaskStatus? status =
        (task == null) ? DownloadTaskStatus.undefined : task.status;
      ...
      children: [
        GestureDetector(
          child: SizedBox(
            width: 170,
            height: 120,
            child: Image.network('$apiBase/assets/messages/${attach.fileUrl}'),
          ),
          onTap:
              () =>
                  attach.fileType == '2'
            ...
            TextButton(
              ...
              child: Text(
                attach.fileName.length > 18
                    ? '${attach.fileName.substring(0, 14)}...${attach.fileName.substring(attach.fileName.length - 4)}'
                    : attach.fileName,
              ),
            ),
            Row(
              children: [
                ...
                if (status == DownloadTaskStatus.running)
                  AdminBtnCircleIcon(
                    tapFunc: () => _pauseDownload(task!),
                    icon: Icons.pause,
                  ),
                if (status == DownloadTaskStatus.paused)
                  AdminBtnCircleIcon(
                    tapFunc: () => _resumeDownload(task!),
                    icon: Icons.restore,
                  ),
                if (status == DownloadTaskStatus.failed)
                  AdminBtnCircleIcon(
                    tapFunc: () => _retryDownload(task!),
                    icon: Icons.refresh,
                  ),
                if (status != DownloadTaskStatus.undefined)
                  AdminBtnCircleIcon(
                    tapFunc: () => _cancelDownload(task!),
                    icon: Icons.close,
                  ),
              ],
            ),
  ...
  Future<bool> sendMessageControl(
    ...
    await Webservice()
        .loadHttp(context, apiSendMessageUrl, {
