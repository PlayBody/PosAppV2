...
class AdminAdviseAnswer extends StatefulWidget {
  ...
  const AdminAdviseAnswer({required this.adviseId, super.key});

  @override
  State<AdminAdviseAnswer> createState() => _AdminAdviseAnswer();
}
...
class _AdminAdviseAnswer extends State<AdminAdviseAnswer> {
  ...
  ChewieController? aController;
  ChewieController? qController;
  File? _videoFile;
  ...
  Future<List> loadInitData() async {
    ...
    if (results['isLoad']) {
      ...
      if (results['advise']['answer_movie_file'] != null) {
        var vController = VideoPlayerController.networkUrl(
          Uri.parse(adviseMovieBase + results['advise']['answer_movie_file']),
        );
  ...
  Widget _getAdviseTop() {
    return Row(
      children: [
        SizedBox(width: 100, child: Text(questionDate)),
        SizedBox(child: Text(teacherName, style: stylePageSubtitle)),
      ],
    );
  }
  ...
  Widget _getMovieView() {
    ...
    return SizedBox(
      width: 170,
      height: 190,
      child: Chewie(controller: qController!),
    );
  }
  ...
  Widget _getAnswerMovieView() {
    ...
    return SizedBox(
      width: 170,
      height: 190,
      child: Chewie(controller: aController!),
    );
  }
