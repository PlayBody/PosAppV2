...
class AdminCuponList extends StatefulWidget {
  const AdminCuponList({super.key});

  @override
  State<AdminCuponList> createState() => _AdminCuponList();
}
...
class _AdminCuponList extends State<AdminCuponList>
  ...
  Future<List> loadCouponList() async {
    coupons = await ClCoupon().loadCoupons(
      context,
      globals.companyId,
      isGetDelete: true,
    );
                  ...
                  image: DecorationImage(
                    image:
                        e.iconUrl == null
                            ...
                            : NetworkImage(
                              "$apiBase/assets/images/coupons/${e.iconUrl!}",
                            ),
                    fit: BoxFit.cover,
                  ),
                ),
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    if (e.couponCode != '')
                      Text(e.couponCode, style: styleContent),
                    Container(
                      padding: paddingContentLineSpace,
                      child: Text(
                        '有効期限: ${e.useDate.replaceAll('-', '/')}',
                        style: styleContent,
                      ),
                    ),
                    Container(
                ...
                child: Column(
                  children: [
                    if (e.discountAmount != null)
                      Text(
                        '${Funcs().currencyFormat(e.discountAmount!)}円 OFF',
                        style: TextStyle(
                          fontSize: 20,
                    ...
                    if (e.discountRate != null && e.upperAmount != null)
                      Text(
                        '上限${Funcs().currencyFormat(e.upperAmount!)}円',
                        style: TextStyle(
                          fontSize: 20,
                          fontWeight: FontWeight.bold,
                        ),
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 20),
                  Text('【発送履歴】'),
                  if (e.staffs.isEmpty) Text('なし'),
                  ...e.staffs.map(
                    (eStaff) => Text(
                      eStaff.staffNick != ''
                          ? eStaff.staffNick
                          : ('${eStaff.staffFirstName!} ${eStaff.staffLastName!}'),
                    ),
                  ),
                ],
              ),
                ...
                child: TextButton(
                  child: Row(
                    children: [
                      Text('詳細を見る'),
                      Icon(
                        openDetailList.contains(e.couponId)
