...
class ShiftHelper {
  ...
  Future<bool> autoShiftSet(context, organId, fromTime, toTime) async {
    var datas = await ClShift().loadShiftManagePsg(
      context,
      organId,
      fromTime,
      toTime,
    ...
    List<StaffListModel> staffs = await ClStaff().loadStaffs(context, {
      'organ_id': organId,
    });
    ...
    for (ShiftManageModel data in datas) {
      plans[data.fromTime.weekday - 1].appendPlan(
        data.fromTime,
        data.toTime,
        data.count,
      );
